view: member_engagement_metric_daywise {
  ...
  derived_table: {
    ...
    SELECT date,
           ...
           (SELECT count(*) FROM heapdata.pageviews p left join heapdata.users  u on  p.user_id = u.user_id  where DATEADD(day, -1, date_trunc('day', time)) = c.date and  (u._email not like '%wipro.com%' AND u._email not like '%appirio.com%' AND u._email not like '%topcoder.com%') ) as non_topgear_pageviews,
           ...
           (SELECT count(*)  FROM coder WHERE status = 'A' and (email not like '%wipro.com%' AND email not like '%appirio.com%' AND email not like '%topcoder.com%') and CAST(member_since AS DATE) = c.date) as non_topgear_sign_ups,
        -- (SELECT count(*) FROM project WHERE date_trunc('day', posting_date) = c.date and status_desc not in ('Draft', 'Deleted', 'New', 'Inactive') and client_project_id not in (80000062,80003742, 80003787,80003742, 80003610) ) as non_topgear_challenges,
           ...
           (SELECT count(*) FROM project p left join client_project_dim pd on p.client_project_id = pd.client_project_id WHERE date_trunc('day', p.posting_date) = c.date and p.status_desc not in ('Draft', 'Deleted', 'New', 'Inactive') and p.client_project_id not in (80000062,80003742, 80003787,80003742, 80003610) and p.task_ind = 0 ) as non_topgear_challenges,
           (SELECT count(*) FROM project p left join client_project_dim pd on p.client_project_id = pd.client_project_id WHERE date_trunc('day', p.posting_date) = c.date and p.status_desc not in ('Draft', 'Deleted', 'New', 'Inactive') and p.client_project_id not in (80000062,80003742, 80003787,80003742, 80003610) and p.task_ind = 1 and pd.is_taas_billing_account != true ) as non_topgear_tasks,
           (SELECT count(*) FROM project p left join client_project_dim pd on p.client_project_id = pd.client_project_id WHERE date_trunc('day', p.posting_date) = c.date and p.status_desc not in ('Draft', 'Deleted', 'New', 'Inactive') and p.client_project_id not in (80000062,80003742, 80003787,80003742, 80003610) and p.task_ind = 1 and pd.is_taas_billing_account = true ) as non_topgear_TaaS,
           ...
           (select count(*) from (select dpr.project_id , dpr.user_id , dpr.inquire_timestamp, p.client_project_id from tcs_dw.design_project_result dpr left join tcs_dw.project p on dpr.project_id = p.project_id where p.client_project_id not in (80000062,80003742, 80003787,80003742, 80003610) union select pr.project_id , pr.user_id , pr.inquire_timestamp, p.client_project_id from tcs_dw.project_result pr left join tcs_dw.project p on pr.project_id = p.project_id where p.client_project_id not in (80000062,80003742, 80003787,80003742, 80003610) ) as challenge_registration_union where DATEADD(day,-1,date_trunc('day', inquire_timestamp)) = c.date) as non_topgear_challenge_registrations,
           ...
           (select count(distinct user_id) from (select dpr.project_id , dpr.user_id , dpr.inquire_timestamp, p.client_project_id from tcs_dw.design_project_result dpr left join tcs_dw.project p on dpr.project_id = p.project_id where p.client_project_id not in (80000062,80003742, 80003787,80003742, 80003610) union select pr.project_id , pr.user_id , pr.inquire_timestamp, p.client_project_id from tcs_dw.project_result pr left join tcs_dw.project p on pr.project_id = p.project_id where p.client_project_id not in (80000062,80003742, 80003787,80003742, 80003610) ) as challenge_registration_union where DATEADD(day,-1,date_trunc('day', inquire_timestamp)) = c.date) as non_topgear_distinct_challenge_registrants,
           ...
           (select count(*) from (select dpr.project_id , dpr.user_id , dpr.inquire_timestamp, p.client_project_id, dpr.submit_ind from tcs_dw.design_project_result dpr left join tcs_dw.project p on dpr.project_id = p.project_id where p.client_project_id not in (80000062,80003742, 80003787,80003742, 80003610) union select pr.project_id , pr.user_id , pr.inquire_timestamp, p.client_project_id , pr.submit_ind from tcs_dw.project_result pr left join tcs_dw.project p on pr.project_id = p.project_id where p.client_project_id not in (80000062,80003742, 80003787,80003742, 80003610) ) as challenge_submission_union where DATEADD(day,-1,date_trunc('day', inquire_timestamp)) = c.date and submit_ind = 1 ) as non_topgear_challenge_submissions,
           ...
           (SELECT count(*) FROM recruit_crm_candidate WHERE CAST(created_on AS DATE) = c.date and (email not like '%wipro.com%' AND email not like '%appirio.com%' AND email not like '%topcoder.com%')) as non_topgear_gig_applicants,
           ...
           (SELECT count(distinct u.user_id) FROM user_payment u left join coder co on u.user_id = co.coder_id WHERE CAST(u.create_date AS DATE) = c.date and (co.email not like '%wipro.com%' AND co.email not like '%appirio.com%' AND co.email not like '%topcoder.com%')  ) as non_topgear_members_paid,
           ...
           (SELECT count(*) FROM heapdata.profile_events_success_login WHERE DATEADD(day,-1,date_trunc('day', time)) = c.date  and (email not like '%wipro.com%' AND email not like '%appirio.com%' AND email not like '%topcoder.com%') ) as non_topgear_logins,
           ...
           (SELECT count(distinct nickname) FROM heapdata.profile_events_success_login WHERE DATEADD(day,-1,date_trunc('day', time)) = c.date and (email not like '%wipro.com%' AND email not like '%appirio.com%' AND email not like '%topcoder.com%')  ) as non_topgear_distinct_logins
    FROM calendar c
