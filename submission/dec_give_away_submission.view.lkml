view: dec_give_away_submission {
  derived_table: {
    sql: -- use existing billing_account_budgets in looker_pdt.LR$QHFEMDLRL8UKDCMYAXI3C_billing_account_budgets
      ...
      WHERE (member_submission.created_at  >= TIMESTAMP '2021-12-01' AND member_submission.created_at  <= TIMESTAMP '2022-01-04') AND (NVL(challenge.task_ind, 0)  = 0)
      ...
      where (TRUNC(TIMESTAMP 'epoch' + problem_submission.submit_time/1000 *INTERVAL '1 second') >= '2021-12-01'
            AND TRUNC(TIMESTAMP 'epoch' + problem_submission.submit_time/1000 *INTERVAL '1 second') <= '2022-01-04' )
       ;;
  }
  ...
  dimension: week_number {
    type: string
    sql: CASE
        when ${submission_time_date} > '2021-11-30' AND ${submission_time_date} <= '2021-12-07' THEN 'Week 1'
        when ${submission_time_date} > '2021-12-07' AND ${submission_time_date} <= '2021-12-14' THEN 'Week 2'
        when ${submission_time_date} > '2021-12-14' AND ${submission_time_date} <= '2021-12-21' THEN 'Week 3'
        when ${submission_time_date} > '2021-12-21' AND ${submission_time_date} <= '2021-12-28' THEN 'Week 4'
        when ${submission_time_date} > '2021-12-28' AND ${submission_time_date} <= '2022-01-04' THEN 'Week 5'
        else null
        END;;
  }
